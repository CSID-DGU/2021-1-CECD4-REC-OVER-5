...
namespace RUDY_SCANNER
{
    class Scanner
    {
        static void Main(string[] args)
        {
            ...
            List<bool> fileDownloadList = new List<bool>();
            ...
            foreach (var fileName in files)  //insert metadata loop
            {
                ...
                if (info.AlternateDataStreamExists("Zone.Identifier"))
                    fileDownloadList.Add(true);
                else
                    fileDownloadList.Add(false);
            ...
            foreach (string str in fileNameList)  //리스트이름 바꿔서 다른정보 출력해 볼 수 있음
            {
